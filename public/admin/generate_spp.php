...
if(isset($_GET['preview']) && $_GET['preview']=='1'){
  ...
  } else { // daftar_ulang preview by year only
    if(strlen($y)===4){
...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  elseif($type==='spp' && !preg_match('/^[0-9]{6}$/',$period)){ $err='Pilih Tahun & Bulan'; }
...
<div class="page-shell generate-spp-page">
  ...
  <?php $recentPeriods=[]; $rsP=mysqli_query($conn,"SELECT period, COUNT(*) c, SUM(amount) total FROM invoice WHERE type='".($type==='spp'?'spp':'daftar_ulang')."' GROUP BY period ORDER BY period DESC LIMIT 6"); while($rsP && $r=mysqli_fetch_assoc($rsP)) $recentPeriods[]=$r; $nowY=(int)date('Y'); $yStart=max(2020,$nowY-1); $yEnd=$nowY+3; ?>
...
<script src="<?= url('assets/js/generate_spp.js') ?>" defer></script>
